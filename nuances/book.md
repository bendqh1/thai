## Internal wall and external wall

### Internal wall

ผนัง

### External wall
กำแพง

This words is similar to กำแพงเพชร

## reng and re_eng

เร่ง
Rushing

แรง
Force

## คำอธิบาย versus การอธิบาย

"การอธิบาย" refers more to the act of explaining, while "คำอธิบาย" means the explanation itself.

## มัก and มาก

มัก often

มาก very

## Jagrayan and jagruwan

จักรยาน 
จักรวาล (ศูนย์กลางจักรวาล)

## Fai sound words

* หลายไฟ
* วายฟาย
* ก๊าซไวไฟ

## Finance

* ธนากร (tana-gon) == Treasurers 
* ธนาคาร (tana-kan) == banks

## Misc
* Pam naman and Palm naman
* มีดบาด and บทบาท
* Supap for polite, Kunapap quality, Sukapap for health, mitarapap for friendship, and เสรีภาพ for freedom.
* เรือน and เรื่อง
* Sanya and sanyan and sanyagam
* เชิญ as welcome versus ชวน as I invite (but not necessarily pay)

## NWN words

* สงวน == reserved
* จำนวน == amount
ค* ำนวณ == calculates
ส* ำนวน == idiom

## น้อย and หน่อย

น้อย means just little but หน่อย means little and comes after นิด

## เก่า and แก่

เพื่อนเก่า or เก้าอี้เก่า is not similar to เขาแก่แล้ว or สัตว์แก่แล้ว (this person or is old already or this animal is old already).

## ผู้บริหาร and ผู้จัดการตัวไป

ผู้บริหาร is a superior manager and chief or "on the ground" manager right under the superior manager. 

Both may not be the owner or "shareholder".

## have you eaten yet

If someone already at we will not ask have you eaten yet (กินข้าวหรือยัง).

We will ask:

* อิ่มไหม
* กินอะไรมา

## Lift vrsus lip

When many Thai people refer to an elevator, they will normally name it "lip".

It is written in Thai as `ลิฟต์` which could be read as "lif" or "lift" (with a soft t) but in practice we should say "lip"

## The start of the street and the end of the street

### Start

* ต้น-ถนน
* ไปจุด--เริ่มต้น--ของถนน 
* ไปต้น--ทาง--ถนน

### End

* สุดถนน
* จนถึงสุดถนน

## สิ and เถอ

* `เถอะ` → soft suggestion (also to any number of people). Used in group rehtoric.
* `สิ` → strong suggestion (to any number of people). Used between family members, close friend and couples.

Both encouragements/imperatives can be softened with sentence enders:

* ทำเถอะนะครับ → gentle encouragement, even softer.
* ทำสิครับ → firm request, but polite.

## being_surprised_of_size_differences_of_anything.md

Seller:

แค่ พันสอง เอง

Buyer:

* ตั้ง พันสอง?
* พันสอง แน่ะ !!!
* ตั้ง พันสอง แน่ะ !!!

The buyer is eing surprised from difference of size -- of price in that in case.

## The next life

Consider the following sentence:

> ชีวิตหน้าน่ากังวล

This sentence is comprised of the following parts:

* ชีวิต
* หน้า
* น่า
* กังวล
